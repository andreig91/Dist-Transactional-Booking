...
public class ResourceManagerImpl
	...
	static ResourceManager rmF = null;
	 ...
	 try 
		 ...
		 if(args.length >= 5)
		 {
			// get a reference to the rmiregistry
		     Registry registry = LocateRegistry.getRegistry(args[2], Integer.parseInt(args[4]));
		     // get the proxy and the remote reference by rmiregistry lookup
		     rmF = (ResourceManager) registry.lookup(args[3]);
		     if(rmF!=null)
			 {
		 ...
		 {
			// get a reference to the rmiregistry
		     Registry registry = LocateRegistry.getRegistry(args[5], Integer.parseInt(args[7]));
		     // get the proxy and the remote reference by rmiregistry lookup
		     rmC = (ResourceManager) registry.lookup(args[6]);
		     if(rmC!=null)
			 {
			    System.out.println("Successful");
			    System.out.println("Connected");
		 ...
		 {
			// get a reference to the rmiregistry
		     Registry registry = LocateRegistry.getRegistry(args[8], Integer.parseInt(args[10]));
		     // get the proxy and the remote reference by rmiregistry lookup
		     rmH = (ResourceManager) registry.lookup(args[9]);
		     if(rmH!=null)
			 {
			    System.out.println("Successful");
			    System.out.println("Connected");
			...
			throws RemoteException
	{
		...
		if(key.charAt(0) == 'f')
		{
			price = rmF.reserveItemHelper(id, customerID, key, location);
		}
		else if(key.charAt(0) == 'c')
		{
			price = rmC.reserveItemHelper(id, customerID, key, location);
		}
		else
		{
			price = rmH.reserveItemHelper(id, customerID, key, location);
		}
		if(price == -1)
		{
			return false;
		...
		throws RemoteException
	{
		...
		return(rmF.addFlight(id, flightNum, flightSeats, flightPrice));
	}
	...
	public boolean deleteFlight(int id, int flightNum)
		throws RemoteException
	{
		return rmF.deleteFlight(id, flightNum);
	}
	...
	public int queryFlight(int id, int flightNum)
		throws RemoteException
	{
		return rmF.queryFlight(id, flightNum);
	}
...
//	}
	...
	public int queryFlightPrice(int id, int flightNum )
		throws RemoteException
	{
		return rmF.queryFlightPrice(id, flightNum);
	}
			...
			throws RemoteException
	{
		...
		if( cust == null ) {
			...
			for(Enumeration e = reservationHT.keys(); e.hasMoreElements();){		
				...
				{
					temp = rmF.removeReservation(id, reserveditem.getKey(), reserveditem.getCount());
				}
				else if(reserveditem.getKey().charAt(0) == 'c')
				{
					temp = rmC.removeReservation(id, reserveditem.getKey(), reserveditem.getCount());
				}
				else
				{
					temp = rmH.removeReservation(id, reserveditem.getKey(), reserveditem.getCount());
				}
				if(!temp)
				{
					return false;
...
//	
	...
	throws RemoteException {
    	...
    	if(car)
    	{
    		int carPrice = rmC.reserveItemHelper(id, customer,Car.getKey(location), location);
    		if(carPrice == -1)
    		{
    			return false;
    		}
    	...
    	if(Room)
    	{
    		int roomPrice = rmH.reserveItemHelper(id, customer,Hotel.getKey(location), location);
    		if(roomPrice == -1)
    		{
    			return false;
    		}
    	...
    	while(iterator.hasNext())
    	{
    		...
    		flightPrice = rmF.reserveItemHelper(id, customer, Flight.getKey(flightNum), String.valueOf(flightNum));
    		if(flightPrice == -1)
    		{
    			return false;
    		}
